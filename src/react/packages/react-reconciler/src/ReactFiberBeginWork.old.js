...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  const nextProps = workInProgress.pendingProps; // 本次渲染需要使用的 props
  const prevState = workInProgress.memoizedState; // 上次渲染使用的 props
  const prevChildren = prevState !== null ? prevState.element : null;
  cloneUpdateQueue(current, workInProgress);
  processUpdateQueue(workInProgress, nextProps, null, renderLanes); // 执行一个更新队列
    ...
    if (supportsHydration) {
      ...
      while (node) {
        ...
        node = node.sibling;
              ...
              // root.
              reconcileChildren(current, workInProgress, nextChildren, renderLanes); // 产生fiber子节点
      resetHydrationState();
    }
    return workInProgress.child;
}
...
function updateHostComponent(
  ...
  if (isDirectTextChild) { // 文字类型优化，可以少创造一个text类型的fiber节点，具体来讲就是type为textarea、option、noscript和children为string或number，或者有dangerouslySetInnerHTML的fiber节点
  ...
  markRef(current, workInProgress); // ref标记
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress.child;
}
...
function validateSuspenseListNestedChild(childSlot: mixed, index: number) {
  if (__DEV__) {
    ...
    function beginWork(
      ...
      // 通过current判断是否是update，在初次创建时current是null
      if (current !== null) {
      ...
      switch (workInProgress.tag) {
        ...
        case HostRoot: // root 3，按照现有顺序 为 hostroot -> ClassComponent -> HostComponent
          console.log('updateHostRoot', current, workInProgress, renderLanes)
          return updateHostRoot(current, workInProgress, renderLanes);
        case HostComponent: // 原生标签 5
